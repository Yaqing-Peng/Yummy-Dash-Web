<template>
  <div class="navbar">
    <div class="statusBox">
      ...
      <span v-if="status===1" class="businessBtn">Opening</span>
      <span v-else class="businessBtn closing">Closed</span>
    </div>
    ...
    <div :key="restKey" class="right-menu">
      <div class="rightStatus">
        ...
        <span class="navicon operatingState" @click="handleStatus"><i />Open/Close</span>
      </div>
    ...
    <!-- 营业状态弹层 -->
    <el-dialog title="Business Status Setting" :visible.sync="dialogVisible" width="25%" :show-close="false">
      <el-radio-group v-model="setStatus">
        <el-radio :label="1">
          Opening
          <span>The restaurant is currently open and automatically accepts any orders. You can click to close.</span>
        </el-radio>
        <el-radio :label="0">
          Closed
          <span>The restaurant is currently closed and only accepts pre-orders during business hours. You can click to
            reopen.</span>
        </el-radio>
      </el-radio-group>
...
export default class extends Vue {
  ...
  webSocket() {
    ...
    if (typeof WebSocket == 'undefined') {
      that.$notify({
        title: 'Notice',
        message: 'The current browser cannot receive real-time alarm messages. Please use Google Chrome!',
        type: 'warning',
        duration: 0,
      });
    } else {
      ...
      this.websocket.onmessage = function (msg) {
        ...
        that.$notify({
          title: jsonMsg.type === 1 ? 'Pending Acceptance' : 'Reminder',
          duration: 0,
          ...
          // Here you can also add the returned information to the message for display
          message: `${jsonMsg.type === 1
              ? `<span>You have 1 <span style=color:#419EFF>order pending</span> processing, ${jsonMsg.content}, please accept it in time</span>`
              : `${jsonMsg.content}<span style='color:#419EFF;cursor: pointer'>Go to process</span>`
            }`,
        });
      };
      ...
      this.websocket.onerror = function () {
        that.$notify({
          title: 'Error',
          message: 'Server error, unable to receive real-time alarm messages',
          type: 'error',
          duration: 0,
        });
      };
